...
`;var p=null;function w(e){e.store("lightbox",{show:{},items:{},touchStart:null,onTouchStart(t){if(!t.changedTouches){this.touchStart=null;return}this.touchStart=t.changedTouches[0]},onTouchEnd(t,o){if(!t.changedTouches||!this.touchStart)return;let{screenX:s,screenY:a}=this.touchStart,{screenX:h,screenY:u}=t.changedTouches[0];if(!s||!a||!h||!u)return;let n=s-h,c=a-u;Math.abs(n)<Math.abs(c)||(n>=100?this.next(o):n<=-100&&this.prev(o))},prev(t){let o=this.items[t].findIndex(s=>s.el===this.show[t].el);this.show[t]=o===0?this.items[t][this.items[t].length-1]:this.items[t][o-1]},next(t){let o=this.items[t].findIndex(s=>s.el===this.show[t].el);this.show[t]=o===this.items[t].length-1?this.items[t][0]:this.items[t][o+1]}}),e.directive("lightbox",(t,{value:o,modifiers:s,expression:a},{effect:h,evaluateLater:u})=>{if(o==="group")return;if(!a){console.warn("Alpine warn: no url or config expression passed to x-lightbox",t);return}let n=u(a);h(()=>{n(c=>{let i=b(t,c);if(e.store("lightbox").show[i]===void 0&&(e.store("lightbox").show[i]=null),e.store("lightbox").items[i]??=[],!document.querySelector(`#lightbox-${i}`)){let r=document.createElement("template");r.innerHTML=x;let l=r.content.children[0];l.id=`lightbox-${i}`,l.setAttribute("x-data",`{ group: '${i}' }`),document.body.appendChild(l),setTimeout(()=>{l.hasOwnProperty("_x_isShown")||e.initTree(l)},15)}let m=e.store("lightbox").items,d=e.store("lightbox").items[i]?.findIndex(r=>r.el===t),g=v(c,i,t);d!==-1&&d!==void 0?m[i][d]=g:m[i].push(g),t.addEventListener("click",r=>{r.preventDefault(),e.store("lightbox").show[i]=e.store("lightbox").items[i].find(l=>l.el===t)})})})})}var b=(e,t)=>e.hasAttribute("x-lightbox:group")?e.getAttribute("x-lightbox:group"):t.group?String(t.group):(p??=(Math.random()+1).toString(36).substring(7),p),v=(e,t,o)=>(typeof e=="string"&&(e={url:e}),{el:o,type:"image",autoplay:!1,muted:!1,...e,group:t});document.addEventListener("alpine:init",()=>window.Alpine.plugin(w));})();
