...
`;function p(e){let u=(Math.random()+1).toString(36).substring(7);e.store("lightbox",{show:{},items:{},touchStart:null,onTouchStart(t){if(!t.changedTouches){this.touchStart=null;return}this.touchStart=t.changedTouches[0]},onTouchEnd(t,i){if(!t.changedTouches||!this.touchStart)return;let{screenX:s,screenY:n}=this.touchStart,{screenX:h,screenY:c}=t.changedTouches[0];if(!s||!n||!h||!c)return;let l=s-h,o=n-c;Math.abs(l)<Math.abs(o)||(l>=100?this.next(i):l<=-100&&this.prev(i))},prev(t){let i=this.items[t].findIndex(s=>s.el===this.show[t].el);this.show[t]=i===0?this.items[t][this.items[t].length-1]:this.items[t][i-1]},next(t){let i=this.items[t].findIndex(s=>s.el===this.show[t].el);this.show[t]=i===this.items[t].length-1?this.items[t][0]:this.items[t][i+1]}}),e.directive("lightbox",(t,{modifiers:i,expression:s},{effect:n,evaluateLater:h})=>{if(!s){console.warn("Alpine warn: no url or config expression passed to x-lightbox",t);return}let c=h(s);n(()=>{c(l=>{let o=l.group?String(l.group):u;if(e.store("lightbox").show[o]===void 0&&(e.store("lightbox").show[o]=null),e.store("lightbox").items[o]??=[],!document.querySelector(`#lightbox-${o}`)){let r=document.createElement("template");r.innerHTML=x;let a=r.content.children[0];a.id=`lightbox-${o}`,a.setAttribute("x-data",`{ group: '${o}' }`),document.body.appendChild(a),e.initTree(a)}let m=e.store("lightbox").items,d=e.store("lightbox").items[o]?.findIndex(r=>r.el===t),g=f(l,o,t);d!==-1&&d!==void 0?m[o][d]=g:m[o].push(g),t.addEventListener("click",r=>{r.preventDefault(),e.store("lightbox").show[o]=e.store("lightbox").items[o].find(a=>a.el===t)})})})})}var f=(e,u,t)=>(typeof e=="string"&&(e={url:e}),{el:t,type:"image",...e,group:u});document.addEventListener("alpine:init",()=>window.Alpine.plugin(p));})();
